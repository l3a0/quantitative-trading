...
1. [vintages.jsonl](vintages.jsonl) is the record. One JSON object per line,
   ...
   four name their vendor `chan-xls` and carry a `source_workbook` field
   ...
   two happens to spell all four of these files and spells the wrong workbook
   for a column whose source is named after a chapter's example rather than
