...
| Pinning the eight committed vintages by set equality over the whole manifest | The pin compared the whole manifest against a set of eight tuples, so it described the eight and spoke for every entry the manifest would ever hold. A ninth vintage is an extra member and fails it, which made recording one fail the suite and put the five replications that each need a new series behind a choice about weakening an integrity check. It is keyed on the eight paths instead, and still asserts all eight are present. Name what that gives up. Set equality also failed when a ninth entry was forged into the manifest by hand, and a comparison keyed on paths does not. Two assertions still catch a forgery. `test_every_committed_series_has_exactly_one_entry` fails when an entry names no file on disk or repeats a path that is already taken, and the path-identity check below holds a forgery that arrives with its own new file to the name that file took. |
| Writing the symbol into a recorded vintage's file rather than holding it from the path | Scoping the pin above removes the only hold on a new entry's identity, and the four fields [src/chan/vintage.py](../src/chan/vintage.py)'s `vintage_filename` puts in a path go unheld: vendor, symbol, price basis and download date. A manifest naming the wrong series then reads green, and the reader hands one series' closes back under another's name, verified against the recorded sha256, because a hash is a claim about bytes and says nothing about which series they are. The span, the row count and the sha256 stay held, because `test_every_entry_describes_the_file_it_names` derives all three from the file. Writing the symbol into the bytes would hold it, and it costs four surfaces that pin the recorder's single `Date,Close` header: two byte-for-byte pins, the assertion that the header does not borrow one vendor's shape, and the prose in `_serialize` and [data/README.md](../data/README.md)'s `## Header shape`. The reader does not constrain the choice, since `_parse_close` drops every leading row whose first field is not a date and reads either shape. What decides it is when the decision is made. No recorded vintage exists yet, so today the change costs those four surfaces and nothing else, and after the first one is recorded a vintage is immutable by the premise, so the older shape stays on disk and the set of files a check has to exempt grows by an entry nothing marks as older. The path already carries all five identity fields, so comparing an entry against the name it took is the cheaper hold and it moves no bytes. That check is not the path-naming the two rows above cut. Nothing is parsed out of a filename and no caller gains a path argument, the comparison running from the record to its shadow rather than back. The cost those rows name still lands. A check that asserts the convention is a check that has to move if the convention does. The eight are exempt, because they predate the recorder and the rename row above keeps their hand-given names, so two naming conventions coexist on purpose and the exemption set is named rather than incidental. |
