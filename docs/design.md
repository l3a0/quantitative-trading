...
| The script that draws the regime-map figure | The scan behind the figure came across and is pinned, so the picture is presentation rather than a result. The figure itself arrived with the essay when the owner reversed that cut, and the generator did not. The objection the cut raised now stands against the committed image: nothing here regenerates or checks it, and [issue 6](https://github.com/l3a0/quantitative-trading/issues/6) names a checked-in generated artifact as one of the three things that reopen the sweep policy. |
